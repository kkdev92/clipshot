...
- **`Ctrl+Shift+V` does nothing while the integrated terminal has focus**: A key press in the terminal belongs to the program running there unless the command is listed in `terminal.integrated.commandsToSkipShell`. ClipShot adds itself to that list on activation, so this should be handled — if it is not, run **ClipShot: Enable Shortcut in Integrated Terminal** from the Command Palette, which reports what it found. The list is only consulted when `clipshot.terminal.registerShortcut` is left on, and an entry reading `-clipshot.pasteImage` is treated as a deliberate opt-out and left alone
